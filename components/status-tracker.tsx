...
export function StatusTracker({ estatusActual, onStatusUpdate }: StatusTrackerProps) {
  const currentStepIndex = estatusSteps.findIndex(step => step.key === estatusActual);
                ...
                return (
                  <div key={step.key} className="flex items-center">
                    <div className="flex flex-col items-center">
                      <div className={`w-12 h-12 rounded-full border-2 flex items-center justify-center ${getStepColor(status)}`}>
                    ...
                    {!isLast && (
                      <div className="flex-1 mx-4">
                        <div className={`h-0.5 w-full ${index < currentStepIndex ? 'bg-emerald-600' : 'bg-gray-200'}`}></div>
                      </div>
                    )}
                  </div>
                );
